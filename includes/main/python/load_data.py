...
def load_delta_table(spark: SparkSession, filename: str,
                     landing_path: str, delta_table_path: str) -> bool:
    "Load a parquet file as a Delta table."
    parquet_df = spark.read.format("parquet").load(landing_path + filename)
...
def load_data(filename: str, kind: str, username: str) -> bool:
    ...
    load_delta_table(spark, filename, landing_path, table_path)
